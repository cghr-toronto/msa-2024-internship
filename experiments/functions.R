...
pdf_print <- function(series, title, width, height){
    ...
    ggsave(jpeg_title, plot = series, device = "jpeg", width = width, height = height, limitsize = FALSE, dpi = 400)
...
hm <- function(ns_table, hm_title, pdf_title, labels = TRUE, cod_order, cod_custom_order = NULL, symp_order, symp_custom_order = NULL, keep_only = TRUE, symptoms, width, height) {
    ...
    if (keep_only){
        ns_table <- ns_table %>% select(all_of(symptoms), cause_of_death, deaths)
    }
    ...
    out <- pdf_print(heat_map_plot, pdf_title, width = width, height = height)
