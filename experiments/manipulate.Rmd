...
shelters_old <- st_read("data/Shelter/shelters_wgs84.shp")
shelters <- shelters_old  %>% st_set_crs(NA) %>% st_set_crs(3857)
shelters <- shelters_old  %>% st_transform(3857)

# Reading in the Neighbourhoods GeoJson File
ngh_old <- st_read("data/Neighbourhoods.geojson")
ngh <- ngh_old  %>% st_set_crs(NA) %>% st_set_crs(3857)
ngh <- ngh_old  %>% st_transform(3857)

# Reading in the Neighbourhood Improvement Areas GeoJson File
ngh_imp_old <- st_read("data/Neighbourhood Improvement Areas.geojson")
ngh_imp <- ngh_imp_old  %>% st_set_crs(NA) %>% st_set_crs(3857)
ngh_imp <- ngh_imp_old  %>% st_transform(3857)
```
