...
waste_old <- st_read("data/Waste/SW_transfer_station_wgs84.shp")
waste <- waste_old  %>% st_set_crs(NA) %>% st_set_crs(3857)
waste <- waste_old  %>% st_transform(3857)
```
...
```{r}
# Joining the waste file to the shelter file
joined <- st_nearest_points(waste, shelters, left = TRUE, right = TRUE)
```
